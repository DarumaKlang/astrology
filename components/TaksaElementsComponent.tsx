...
import { taksaElementsData } from '../data/mahataksaData';

const TaksaElementsComponent: React.FC = () => {
    ...
    return (
        <ThemedSection title={taksaElementsData.name} gridColsClassName="grid-cols-1">
            <ThemedCard className="flex flex-col items-center justify-center p-4">
                <div className="grid grid-cols-3 gap-0 w-full max-w-xs overflow-hidden rounded-md">
                    {gridItems(taksaElementsData.numbers, taksaElementsData.elements).map((item, itemIndex) => (
                        <div
                            key={itemIndex}
                            className={`
...
export default TaksaElementsComponent;