// src/components/TaksaElementsComponent.tsx
import React from 'react';
import ThemedSection from './ThemedSection';
import ThemedCard from './ThemedCard';
import { taksaPowerData } from '../data/mahataksaData';

const TaksaPowerComponent: React.FC = () => {
    ...
    return (
        <ThemedSection title={taksaPowerData.name} gridColsClassName="grid-cols-1">
            <ThemedCard className="flex flex-col items-center justify-center p-4">
                <div className="grid grid-cols-3 gap-0 w-full max-w-xs overflow-hidden rounded-md">
                    {gridItems(taksaPowerData.numbers, taksaPowerData.power).map((item, itemIndex) => (
                        <div
                            key={itemIndex}
                            className={`
                        ...
                        >
                            {item.num !== null ? (
                                <>
                                    ...
                                    <div className="text-xs font-normal mt-1 text-gray-400">{item.text}</div>
                                </>
                            ) : (
                                ''
                            )}
...
export default TaksaPowerComponent;